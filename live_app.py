...
st.set_page_config(page_title="Diving Evaluation", page_icon="ð¤¿")

# Auth handled by login_view()
# --- Caching fÃ¼r selten geÃ¤nderte Tabellen ---
@st.cache_data
...
def get_official_category_local(age, year, agecat_df):
    try:
        ...
        return None
    # Falls du ein Jahr-Feld hast, ergÃ¤nze: & (agecat_df["year"] == int(year))
...
def get_points(discipline_id, result, category, sex):
    try:
        ...
        if result in (None, "", " ", "nan"):
            return 0
        # NEU: Wenn raw_result 9999 ist, immer 0 Punkte zurÃ¼ckgeben
    ...
    if st.button("Einloggen") and email and password:
        try:
            ...
            if password_ok and email_ok:
                ...
                st.error("Login fehlgeschlagen â E-Mail oder Passwort falsch.")
...
def startseite():
    ...
    with col3:
        if st.button("Athleten bearbeiten"):
            ...
            st.rerun()
    with col4:
        if st.button("Athleten lÃ¶schen"):
            st.session_state["page"] = "Athleten lÃ¶schen"
            st.rerun()
    ...
    with col3:
        if st.button("Vergleich BIG Competitions"):
            ...
            st.rerun()
    with col4:
        if st.button("Selektionen WettkÃ¤mpfe"):
            st.session_state["page"] = "Selektionen WettkÃ¤mpfe"
            st.rerun()
...
def manage_results_entry():
    st.header("ð¯ Ergebnisse fÃ¼r einen Athleten eingeben")
    ...
    if not athlete_names:
        st.warning("Keine Athleten mit Vor- und Nachnamen gefunden! PrÃ¼fe die Datenbank und die Feldnamen.")
        return

    selected_athlete_name = st.selectbox("WÃ¤hle einen Athleten", list(athlete_names.keys()))
    ...
    if not test_year.isdigit() or len(test_year) != 4:
        st.error("Bitte ein gÃ¼ltiges Jahr im Format yyyy eingeben!")
        return
    ...
    if selected_athlete_name:
        ...
        st.subheader(f"Ergebnisse fÃ¼r {selected_athlete_name} ({category}, {sex}) eingeben")
        ...
        if st.button("ð¾ Ergebnisse speichern"):
            excluded_ids = {
            ...
            st.success("â Ergebnisse gespeichert und Punkte berechnet!")

    st.markdown("---")
    st.subheader("ð¤ Ergebnisse per Datei importieren")
    ...
    if uploaded_file:
        ...
        if not all(col in df.columns for col in expected_base):
            st.error(f"â Die Datei muss folgende Spalten enthalten:\n\n{', '.join(expected_base)}")
            return
        ...
        st.success(f"â {inserted_count} Ergebnisse importiert.")
        if skipped_rows:
            st.warning(f"â ï¸ {len(skipped_rows)} Zeile(n) konnten keinem Athleten zugeordnet werden:")
            st.dataframe(pd.DataFrame(skipped_rows))

# Athleten bearbeiten
def edit_athletes():
    st.header("âï¸ Athleten bearbeiten")
    athletes = db.table_select('athletes')
    athlete_names = {f"{a['first_name']} {a['last_name']}": a['id'] for a in athletes}
    selected_name = st.selectbox("Athlet auswÃ¤hlen", list(athlete_names.keys()))
...
def auswertung_starten():
    st.header("ð Piste Resultate anzeigen")
    ...
    selected_years = dynamic_multiselect("ð Testjahr wÃ¤hlen", all_years, "jahr")
    selected_categories = dynamic_multiselect("ð Kategorie wÃ¤hlen", all_categories, "kategorie")
    selected_sexes = dynamic_multiselect("â§ Geschlecht wÃ¤hlen", all_sexes, "geschlecht")
    selected_names = dynamic_multiselect("ð¤ Name wÃ¤hlen", all_names, "name")
    ...
    st.download_button("ð¥ CSV herunterladen", df.to_csv(index=False, encoding='utf-8-sig'), file_name="resultate.csv", mime='text/csv')
    ...
    try:
        ...
        excel_buffer.seek(0)
        st.download_button("ð¥ Excel herunterladen", excel_buffer, file_name="resultate.xlsx")
    except ImportError:
        st.info("ð¦ Modul 'openpyxl' ist nicht installiert â Excel-Export nicht verfÃ¼gbar.")


def manage_scoretable():
    st.header("ð Scoretabelle verwalten")

    disciplines = db.table_select('pistedisciplines', 'id, name')
    discipline_map = {d['name']: d['id'] for d in disciplines}
    selected_discipline = st.selectbox("Disziplin auswÃ¤hlen", list(discipline_map.keys()))
    ...
    if selected_discipline:
        ...
        st.subheader(f"Aktuelle Punktebereiche fÃ¼r {selected_discipline}")
        if entries:
            for entry in entries:
                with st.expander(f"Bearbeiten: {entry['category']} / {entry['sex']} | {entry['result_min']} - {entry['result_max']} â {entry['points']} Punkte"):
                    ...
                    col1, col2 = st.columns(2)
                    with col1:
                        if st.button("ð¾ Speichern", key=f"save_{entry['id']}"):
                            if new_max >= new_min:
                                ...
                                st.error("â 'Bis' muss grÃ¶Ãer oder gleich 'Von' sein.")
                    with col2:
                        if st.button("ðï¸ LÃ¶schen", key=f"delete_{entry['id']}"):
                            db.table_delete('scoretables', id=entry['id'])
                            st.warning("Eintrag gelÃ¶scht.")
                            st.rerun()
        else:
            st.info("Noch keine Punktebereiche vorhanden fÃ¼r diese Disziplin.")

        st.subheader("â Neuen Eintrag hinzufÃ¼gen")
        ...
        if st.button("â Eintrag speichern", key="add_save"):
...
def import_athletes():
    st.header("ð¥ Athleten importieren")

    # Button fÃ¼r Bioage-Update ALLER Athleten
    if st.button("ð Bioage fÃ¼r alle bestehenden Athleten berechnen und speichern"):
        ...
        for a in athletes:
            ...
            if bioage:
                ...
                skipped += 1
        st.success(f"Bioage fÃ¼r {updated} Athleten aktualisiert. {skipped} Athleten Ã¼bersprungen (fehlendes oder ungÃ¼ltiges Geburtsdatum).")

    uploaded_file = st.file_uploader("CSV-Datei mit Athletendaten hochladen", type="csv")

    # ð Beispiel-CSV zum Herunterladen anbieten
    ...
    st.download_button(
        label="ð Beispiel-CSV herunterladen",
    ...
    if uploaded_file is not None:
        ...
        if not required_columns.issubset(df.columns):
            st.error(f"â Die Datei muss folgende Spalten enthalten: {', '.join(required_columns)}")
            return
        ...
        for _, row in df.iterrows():
            try:
                ...
                # PrÃ¼fen, ob Athlet bereits existiert
                ...
                st.warning(f"Fehler beim EinfÃ¼gen von {row['first_name']} {row['last_name']}: {e}")
        ...
        st.success(f"â {inserted} Athleten erfolgreich importiert.")

def delete_athlete():
    st.header("ðï¸ Athlet lÃ¶schen")
    ...
    selected_name = st.selectbox("WÃ¤hle einen Athleten zum LÃ¶schen", list(athlete_names.keys()))
    ...
    if selected_name:
        ...
        if st.button("â EndgÃ¼ltig lÃ¶schen"):
            try:
                # Ergebnisse lÃ¶schen
                db.table_delete('pisteresults', athlete_id=athlete_id)
                db.table_delete('athletes', id=athlete_id)
                st.success(f"Athlet '{selected_name}' und alle zugehÃ¶rigen Ergebnisse wurden gelÃ¶scht.")
                st.rerun()
            except Exception as e:
                st.error(f"Fehler beim LÃ¶schen: {e}")

def punkte_neuberechnen():
    st.header("ð Punkte neu berechnen fÃ¼r ein bestimmtes Testjahr")

    st.info("""
    **Hinweis:**  
    âPiste Punkte neu berechnenâ wird **nur benÃ¶tigt**, wenn sich etwas an den Bewertungsgrundlagen Ã¤ndert, z.B.:

    - Die Scoretabelle (`scoretables`) wird angepasst (z.B. neue Punkteverteilung, neue Kategorien, neue Altersgrenzen).
    - Die Alterskategorien Ã¤ndern sich.
    - Es gibt sonstige RegelÃ¤nderungen, die die Punkteberechnung beeinflussen.
    ...
    **Nur wenn sich die Regeln nachtrÃ¤glich Ã¤ndern, mÃ¼ssen die bestehenden Ergebnisse mit âPiste Punkte neu berechnenâ aktualisiert werden.**
    """)
    ...
    selected_year = st.selectbox("ð Testjahr fÃ¼r Neuberechnung wÃ¤hlen", all_years)

    if st.button("ð Neuberechnung starten"):
        results = fetch_all_rows("pisteresults", select="*")
        if not results:
            st.warning(f"â ï¸ Keine Resultate fÃ¼r das Jahr {selected_year} gefunden.")
            return
        ...
        # IDs fÃ¼r Spezialdisziplinen holen
        ...
        # 2. FÃ¼r jeden Athleten im Jahr: Spezialdisziplinen berechnen und speichern
        ...
        st.success(f"â {updated_count} Resultate fÃ¼r das Jahr {selected_year} wurden neu bewertet.")

def bewertung_wettkampf():
    st.header("ð Wettkampfbewertungen berechnen")
    ...
    if st.button("ð Alle Wettkampfbewertungen berechnen"):
        ...
        for _, row in df_results.iterrows():
            ...
            if dives is None:
                st.warning(f"Keine dives fÃ¼r {sex}, {category}, {discipline}")
            if points in (None, "", "nan"):
                st.warning(f"Keine Punkte fÃ¼r {row}")
    ...
    if selected_pisteyear and st.button(f"ð Nur PisteYear {selected_pisteyear} neu berechnen"):
        comp_results = fetch_all_rows('compresults')
        df_results = pd.DataFrame(comp_results)
        df_selection = pd.DataFrame(selection_points)
        df_comp = pd.DataFrame(competitions)
        ...
        st.success(f"â {updated_count} Resultate fÃ¼r PisteYear {selected_pisteyear} wurden neu berechnet.")
        st.info(
        ...
        if seen_regional_labels:
            st.info("Gefundene selectionpoints-Competition Labels fÃ¼r Regional: " + ", ".join(sorted(seen_regional_labels)))
        if missing_selection_combos:
            ...
            st.warning("FÃ¼r diese (sex/Discipline/CategoryStart) Kombinationen gibt es keine passenden selectionpoints â NationalTeam/RegionalTeam bleibt immer 'no'.")
            st.dataframe(df_missing)

    if st.button("ð Nur neue EintrÃ¤ge berechnen"):
        comp_results = fetch_all_rows('compresults')
        ...
        for _, row in df_results.iterrows():
            ...
            db.table_update('compresults', update_payload, id=comp_id)
        st.success("Neue EintrÃ¤ge wurden berechnet!")

    # TESTTOOL: Timestamps zurÃ¼cksetzen
    with st.expander("ð§ª Test-Tools"):
        if st.button("â Alle Timestamps in compresults zurÃ¼cksetzen"):
            try:
                comp_results = fetch_all_rows("compresults")
                for row in comp_results:
                    db.table_update('compresults', {"timestamp": None}, id=row["id"])
                st.success("Alle Timestamps wurden zurÃ¼ckgesetzt.")
            except Exception as e:
                st.error(f"Fehler beim ZurÃ¼cksetzen: {e}")


def auswertung_wettkampf():
    st.header("ð Wettkampfauswertungen")

    # Button zur Bewertungsseite
    if st.button("ð Zu Wettkampf-Bewertung"):
    ...
    # Filter fÃ¼r die wichtigsten Felder
    with st.expander("ð Filter anzeigen"):
        first_name_filter = st.text_input("Vorname (Teilstring mÃ¶glich)", "")
        last_name_filter = st.text_input("Nachname (Teilstring mÃ¶glich)", "")
    ...
    st.download_button("ð¥ Gefilterte Ergebnisse als CSV", filtered.to_csv(index=False, encoding='utf-8-sig'),
    ...
    excel_buffer.seek(0)
    st.download_button(
        "ð¥ Gefilterte Ergebnisse als Excel",
        excel_buffer,
...
def manage_compresults_entry():
    ...
    if st.session_state.get("show_new_comp_form", False):
        ...
        if st.button("ð¾ Wettkampf speichern", key="save_competition"):
            if not comp_name:
    ...
    # --- Athleten und WettkÃ¤mpfe laden ---
    ...
    if st.button("ð¾ Ergebnis speichern"):
        if athlete_data:
    ...
    st.download_button("ð Beispiel-Datei herunterladen", example_df.to_csv(index=False).encode("utf-8"), file_name="beispiel_wettkampfresultate.csv", mime="text/csv")
    ...
    if uploaded_file:
        ...
        if not all(col in df.columns for col in required_cols):
            st.error(f"â Die Datei muss folgende Spalten enthalten: {', '.join(required_cols)}")
            return
        ...
        st.success(f"â {inserted} Resultate importiert.")
        if skipped:
    ...
    st.download_button(
        "ð Beispiel DiveLive CSV herunterladen",
    ...
    if not competition_names:
        st.warning("Keine WettkÃ¤mpfe in der Tabelle competitions vorhanden. Bitte zuerst einen Wettkampf anlegen.")
        return

    selected_competition_divelive = st.selectbox(
        "Wettkampf fÃ¼r DiveLive Import",
        competition_names,
    ...
    if divelive_file is not None:
        if st.button("Import DiveLive", key="import_divelive_btn"):
            try:
                ...
                if missing:
                    st.error(f"â DiveLive CSV muss folgende Spalten enthalten: {', '.join(required_src)}")
                ...
                for _, row in df_dl.iterrows():
                    try:
                        ...
                        if points_val is None:
                            skipped_dl.append({"first_name": first, "last_name": last, "reason": "total_award/Points leer/ungÃ¼ltig"})
                            continue
                        ...
                        # Team-Flags berechnen (falls mÃ¶glich) â Import soll nie komplett abstÃ¼rzen
                ...
                st.success(f"â {inserted_dl} DiveLive Resultate importiert (Wettkampf: {selected_competition_divelive}).")
                if skipped_dl:
                    st.warning("Einige Zeilen wurden Ã¼bersprungen:")
                    st.dataframe(pd.DataFrame(skipped_dl))
            except Exception as e:
                st.error(f"â Import fehlgeschlagen: {e}")
...
def piste_refpoint_wettkampf_analyse():
    st.header("ð Piste RefPoint Wettkampf Analyse")
    ...
    st.info("""
    **Hinweis:**  
    Der Button **"Full Analyse"** berechnet fÃ¼r das angegebene Jahr:
    - die RefPoints (Prozent-Erreichung zum Referenzwert) fÃ¼r alle Wettkampfergebnisse,
    - die Top 3 WettkÃ¤mpfe pro Athlet,
    - und die Entwicklung (Vergleich zum Vorjahr), **sofern mindestens fÃ¼r das angegebene Jahr und das Jahr davor Daten vorhanden sind**.
    """)

    years = [str(y) for y in range(2024, 2031)]
    selected_year = st.selectbox("Jahr fÃ¼r Analyse wÃ¤hlen", years)
    ...
    if st.button("Full Analyse"):
        ...
        if refpoints_df.empty or "Discipline" not in refpoints_df.columns:
            st.error("â Tabelle 'pisterefcomppoints' ist leer oder hat falsche Spalten. Bitte Daten neu importieren (_fix_pisterefcomppoints.py ausfÃ¼hren).")
            return
        ...
        for row in compresults:
            ...
            # Nur WettkÃ¤mpfe mit passendem PisteYear verarbeiten!
            ...
            # Sammle Update fÃ¼r Batch â % nur neu schreiben wenn noch nicht vorhanden
        ...
        st.success(f"Berechnen abgeschlossen. {updated} EintrÃ¤ge fÃ¼r {selected_year} aktualisiert.")
        ...
        if ref_col not in df.columns:
            ...
            # --- NEU: Elite ausschlieÃen ---
            df = df[df["CategoryStart"].str.strip().str.lower() != "elite"]

            # --- NEU: Nur WettkÃ¤mpfe mit PisteYear == ausgewÃ¤hltes Jahr ---
            ...
            for (first, last), group in grouped:
                ...
                inserted += 1
            st.success(f"Top3-Auswertung abgeschlossen. {inserted} EintrÃ¤ge fÃ¼r {selected_year} gespeichert.")
        ...
        if not refcompresults:
            st.warning("Keine Daten in pisterefcompresults fÃ¼r die gewÃ¤hlten Jahre gefunden.")
        else:
            ...
            for (first, last), group in grouped:
                ...
                db.table_update('pisterefcompresults', {"quality": quality},
                    ...
                    last_name=this_year_row.iloc[0]["last_name"],
                    PisteYear=int(selected_year))
            st.success(f"Entwicklung fÃ¼r {updated} Personen berechnet und gespeichert.")

def show_top3_wettkaempfe():
    st.header("ð Top 3 WettkÃ¤mpfe pro Athlet und Jahr")
    ...
    if st.button("Show Results"):
        ...
        if top3_df.empty:
            st.info("Keine Top-3-WettkÃ¤mpfe fÃ¼r die Auswahl gefunden.")
            return
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            top3_df.to_excel(writer, index=False, sheet_name="Top 3 WettkÃ¤mpfe")
        output.seek(0)
        st.download_button(
            "ð¥ Top 3 WettkÃ¤mpfe als Excel",
            output.getvalue(),
...
def manage_tool_environment():
    st.header("ð ï¸ Tool Environment Werte eingeben oder importieren")
    ...
    st.download_button("ð Beispiel-CSV herunterladen", example.to_csv(index=False).encode("utf-8"), file_name="tool_environment_beispiel.csv", mime="text/csv")
    ...
    if uploaded_file:
        ...
        if not required_cols.issubset(df.columns):
            ...
            inserted = 0
            skipped = []
            # Athleten-Liste fÃ¼r Lookup laden
...
def bio_mirwald():
    ...
    selected_name = st.selectbox("Athlet auswÃ¤hlen", athlete_names)
    ...
    if st.button("Speichern"):
        ...
        st.success(f"Eintrag fÃ¼r {selected_name} gespeichert.")
    ...
    if uploaded_file is not None:
        ...
        st.success(f"{inserted} EintrÃ¤ge erfolgreich importiert.")
        if missing_athletes:
...
def manage_trainingsperformance_resilienz():
    st.header("ðª Trainingsperformance - Resilienz")
    ...
    if uploaded_file:
        ...
        for _, row in df.iterrows():
            ...
            # PrÃ¼fe, ob Athlet existiert (alle drei Felder mÃ¼ssen stimmen)
            found = any(
                (str(row['first_name']).strip().lower() == a['first_name'].strip().lower() and
                str(row['last_name']).strip().lower() == a['last_name'].strip().lower() and
                str(row.get('birthdate', '')).strip() == str(a['birthdate']))
...
def soc_full_calculation():
    st.header("ð¢ SOC Full Calculation")
    agecategories = db.table_select('agecategories', '*')
    years = [str(y) for y in range(2024, 2031)]
    selected_year = st.selectbox("PisteYear wÃ¤hlen", years)
    if st.button("SOC Full Calculation starten"):
        ...
        st.success(f"Berechnung abgeschlossen und alle EintrÃ¤ge fÃ¼r {selected_year} aktualisiert.")

def show_full_piste_results_soc():
    st.header("ð Full PISTE Results SOC")
    ...
    # Nur gewÃ¼nschte Spalten anzeigen (inkl. CompPointsNationalTeam und talentcard)
    show_cols = [
    ...
    # FÃ¼ge fehlende Spalten als leere Spalten hinzu (fÃ¼r robustes Verhalten)
    for col in show_cols:
    ...
    st.dataframe(filtered)
    st.download_button(
        "ð¥ Gefilterte Ergebnisse als CSV",
    ...
    # --- Grafik: Kurve und Durchschnittswert fÃ¼r Totalpunkte ---
    if not filtered.empty and "totalpoints" in filtered.columns:
        ...
        ax.axhline(mean_val, color="red", linestyle="--", label=f"Ã Totalpunkte: {mean_val:.1f}")
    ...
    # --- Grafik fÃ¼r Talentcard-Verteilung ---
...
def vergleich_big_competitions():
    st.header("ð Vergleich Big Competitions")
    ...
    if compresultsbig.empty or compresults.empty or competitions.empty:
        st.info("Nicht genÃ¼gend Daten vorhanden.")
    ...
    selected_name = st.selectbox("Person auswÃ¤hlen", [""] + name_options, index=0)
    if not selected_name:
        st.info("Bitte eine Person auswÃ¤hlen.")
        return
    ...
    if not selected_year_big:
        st.info("Bitte ein Jahr auswÃ¤hlen.")
        return
    ...
    if not selected_competition_big:
        st.info("Bitte einen Vergleichswettkampf auswÃ¤hlen.")
        return
    ...
    selected_year_comp = st.selectbox("Jahr (WettkÃ¤mpfe)", [""] + years_comp, index=0)
    if not selected_year_comp:
        st.info("Bitte ein Jahr fÃ¼r WettkÃ¤mpfe auswÃ¤hlen.")
        return

    # Filter fÃ¼r Person und Jahr
    person_results = compresults[
        (compresults["first_name"] == sel_first) &
        (compresults["last_name"] == sel_last)
    ]
    # Filter competitions auf das gewÃ¤hlte Jahr
    ...
    # Mapping fÃ¼r schnellen Zugriff: (discipline, sex, category, rank) -> points
    big_map = {}
    ...
    for _, comp in competitions_year.iterrows():
        ...
        # Hole alle Ergebnisse dieser Person fÃ¼r diesen Wettkampf
        ...
        for _, res in person_comp.iterrows():
            ...
            # FÃ¼r alle gewÃ¼nschten RÃ¤nge vergleichen
    ...
    # --- CSV-Import fÃ¼r compresultsbig IMMER anzeigen ---
    show_big_comp_import()
    ...
    df_out = df_out[cols]
    st.download_button("ð¥ Vergleich als CSV herunterladen", df_out.to_csv(index=False, encoding='utf-8-sig'), file_name="vergleich_big_competitions.csv", mime="text/csv")
    st.dataframe(df_out)

# Hilfsfunktion fÃ¼r den Importbereich (damit er immer angezeigt wird)
def show_big_comp_import():
    st.markdown("---")
    st.subheader("ð¤ Big Competitions Ergebnisse importieren (CSV)")
    ...
    st.download_button("ð Beispiel-CSV herunterladen", example.to_csv(index=False).encode("utf-8"), file_name="big_compresults_beispiel.csv", mime="text/csv")
    ...
    if uploaded_file:
        ...
        # Lade bestehende EintrÃ¤ge fÃ¼r schnellen Vergleich
        ...
        st.success(f"â {inserted} neue Ergebnisse importiert.")
        if skipped:
            st.warning(f"{len(skipped)} EintrÃ¤ge waren bereits vorhanden und wurden nicht importiert:")
            st.dataframe(pd.DataFrame(skipped))

def athleten_anzeigen():
    st.header("ð¥ Athleten anzeigen & exportieren")
    ...
    st.dataframe(filtered)
    st.download_button("ð¥ Gefilterte Athleten als CSV", filtered.to_csv(index=False, encoding='utf-8-sig'), file_name="athleten.csv", mime="text/csv")

def selektionen_wettkaempfe():
    st.header("ð Selektionen WettkÃ¤mpfe")
    ...
    selected_year = st.selectbox("Jahr wÃ¤hlen", years)
    ...
    # Filter nach Jahr (Competition â PisteYear)
    filtered = [
        r for r in compresults
    ...
    if not df.empty:
        ...
        st.download_button("ð¥ Ergebnisse als CSV herunterladen", df_show.to_csv(index=False, encoding='utf-8-sig'), file_name=f"{selected_tab}_{selected_year}.csv", mime="text/csv")
    else:
        st.info("Keine passenden EintrÃ¤ge gefunden.")

    # --- NEU: Einzigartige Liste (jede Person nur einmal) ---
    st.subheader("ð¤ Einzigartige Liste (jede Person nur einmal)")
    if not df.empty:
        unique_cols = ["first_name", "last_name", "sex", "CategoryStart"]
        df_unique = df.drop_duplicates(subset=unique_cols)[unique_cols]
        st.dataframe(df_unique)
        st.download_button(
            "ð¥ Einzigartige Liste als CSV herunterladen",
            df_unique.to_csv(index=False, encoding='utf-8-sig'),
            file_name=f"{selected_tab}_{selected_year}_einzigartig.csv",
            mime="text/csv"
        )
    else:
        st.info("Keine EintrÃ¤ge fÃ¼r die einzigartige Liste gefunden.")

def referenztabellen_anzeigen():
    st.header("ð Referenz- und Bewertungstabellen")

    # --- Scoretables ---
    st.subheader("ð Scoretabelle")
    ...
    if not score_df.empty and not pistedisciplines.empty:
        ...
        st.download_button("ð¥ Scoretabelle als CSV", score_df.to_csv(index=False, encoding='utf-8-sig'), file_name="scoretables.csv", mime="text/csv")
    else:
        st.info("Keine Daten in scoretables oder pistedisciplines.")

    # --- Selectionpoints ---
    st.subheader("ð¯ Selectionpoints")
    ...
    if not sel_df.empty:
        ...
        st.dataframe(sel_df)
        st.download_button("ð¥ Selectionpoints als CSV", sel_df.to_csv(index=False, encoding='utf-8-sig'), file_name="selectionpoints.csv", mime="text/csv")
    else:
        st.info("Keine Daten in selectionpoints.")

    # --- pistereftrainingtime ---
    st.subheader("â±ï¸ Piste Ref Training Time")
    ...
    if not reftrain_df.empty:
        ...
        st.download_button("ð¥ pistereftrainingtime als CSV", reftrain_df.to_csv(index=False, encoding='utf-8-sig'), file_name="pistereftrainingtime.csv", mime="text/csv")
    else:
        st.info("Keine Daten in pistereftrainingtime.")

    # --- pistereftrainingsince ---
    st.subheader("ð Piste Ref Training Since")
    ...
    if not refsince_df.empty:
        ...
        st.download_button("ð¥ pistereftrainingsince als CSV", refsince_df.to_csv(index=False, encoding='utf-8-sig'), file_name="pistereftrainingsince.csv", mime="text/csv")
    else:
        st.info("Keine Daten in pistereftrainingsince.")

    # --- pisterefminpoints ---
    st.subheader("ð¢ Piste Ref Min Points")
    ...
    if not refmin_df.empty:
        ...
        st.download_button("ð¥ pisterefminpoints als CSV", refmin_df.to_csv(index=False, encoding='utf-8-sig'), file_name="pisterefminpoints.csv", mime="text/csv")
    else:
        st.info("Keine Daten in pisterefminpoints.")

    # --- pisterefcomppoints ---
    st.subheader("ð Piste Ref Comp Points")
    refcomp_df = pd.DataFrame(fetch_all_rows("pisterefcomppoints", select="*"))
    if not refcomp_df.empty:
        # Spalten fÃ¼r Alter 9-18 und quality9-quality18
        ...
        st.download_button("ð¥ pisterefcomppoints als CSV", refcomp_df.to_csv(index=False, encoding='utf-8-sig'), file_name="pisterefcomppoints.csv", mime="text/csv")
    else:
        st.info("Keine Daten in pisterefcomppoints.")

    # --- Piste Points fÃ¼r PisteTotalinPoints ---
    st.subheader("ð Piste Points (PisteTotalinPoints)")
    pistedisciplines = pd.DataFrame(fetch_all_rows("pistedisciplines", select="id,name"))
    ...
    if pistetotalinpoints_id:
        ...
        if not pistepoints_df.empty:
            ...
            st.download_button("ð¥ Piste Points als CSV", pistepoints_df.to_csv(index=False, encoding='utf-8-sig'), file_name="piste_points.csv", mime="text/csv")
        else:
            st.info("Keine Piste Points fÃ¼r PisteTotalinPoints gefunden.")
    else:
        st.info("Disziplin 'PisteTotalinPoints' nicht gefunden.")

# --- CompPerfEnhance (Leistungsentwicklung) ---
    st.subheader("ð Leistungsentwicklung (CompPerfEnhance)")
    ...
    if comp_perf_enhance_id:
        ...
        if not enhance_df.empty:
            ...
            st.download_button("ð¥ Leistungsentwicklung als CSV", enhance_df.to_csv(index=False, encoding='utf-8-sig'), file_name="leistungsentwicklung.csv", mime="text/csv")
        else:
            st.info("Keine Daten fÃ¼r CompPerfEnhance gefunden.")
    else:
        st.info("Disziplin 'CompPerfEnhance' nicht gefunden.")

    # --- CompPerfPointsCalc (Wettkampf Performance) ---
    st.subheader("ð Wettkampf Performance (CompPerfPointsCalc)")
    ...
    if comp_perf_points_id:
        ...
        if not perf_points_df.empty:
            ...
            st.download_button("ð¥ Wettkampf Performance als CSV", perf_points_df.to_csv(index=False, encoding='utf-8-sig'), file_name="wettkampf_performance.csv", mime="text/csv")
        else:
            st.info("Keine Daten fÃ¼r CompPerfPointsCalc gefunden.")
    else:
        st.info("Disziplin 'CompPerfPointsCalc' nicht gefunden.")

    # --- CompPerfQualityCalc (Sprung QualitÃ¤t) ---
    st.subheader("ð¤¸ Sprung QualitÃ¤t (CompPerfQualityCalc)")
    ...
    if comp_perf_quality_id:
        ...
        if not perf_quality_df.empty:
            ...
            st.download_button("ð¥ Sprung QualitÃ¤t als CSV", perf_quality_df.to_csv(index=False, encoding='utf-8-sig'), file_name="sprung_qualitaet.csv", mime="text/csv")
        else:
            st.info("Keine Daten fÃ¼r CompPerfQualityCalc gefunden.")
    else:
        st.info("Disziplin 'CompPerfQualityCalc' nicht gefunden.")

    # --- Total Points for Card ---
    st.subheader("ð Total Points for Card (pisterefminpoints)")
    ...
    if not cardpoints_df.empty:
        ...
        st.download_button("ð¥ Total Points for Card als CSV", cardpoints_df.to_csv(index=False, encoding='utf-8-sig'), file_name="total_points_for_card.csv", mime="text/csv")
    else:
        st.info("Keine Daten in pisterefminpoints (Total Points for Card).")

def athleten_eingeben():
    st.header("ð Neuen Athleten hinzufÃ¼gen")

    # SekundÃ¤re Aktionen direkt hier verfÃ¼gbar machen
    st.markdown("#### âï¸ ZusÃ¤tzliche Aktionen")
    col1, col2 = st.columns(2)
    with col1:
        if st.button("ð¥ Athleten importieren"):
            import_athletes()
    with col2:
        if st.button("âï¸ Athleten bearbeiten"):
            edit_athletes()

    st.markdown("---")

    # Standardformular zum HinzufÃ¼gen
    first_name = st.text_input("Vorname")
    last_name = st.text_input("Nachname")
...
def show_full_piste_results_clubs():
    st.header("ð Full PISTE Results for Clubs")
    ...
    # FÃ¼ge fehlende Spalten als None hinzu
    for k in show_cols:
...
def main():
    ...
    menu = [
        ...
        "Selektionen WettkÃ¤mpfe",
        "Vergleich BIG Competitions",
        "Referenz- und Bewertungstabellen"
    ]
    st.sidebar.title("ð  Navigation")
    selected = st.sidebar.radio("WÃ¤hle eine Seite", menu, index=menu.index(st.session_state["page"]))
    ...
    if selected == "Startseite":
        ...
        edit_athletes()
    elif selected == "Athleten lÃ¶schen":
        delete_athlete()
    ...
    elif selected == "Selektionen WettkÃ¤mpfe":
...
if __name__ == "__main__":
    ...
    if st.session_state["user"]:
        logout_button()
        main()  # <-- Navigation Ã¼ber das HauptmenÃ¼
    else:
        login_view()
